...
// Community API session auth (v7.2.0 protocol): a one-time browser
// verification issues a session id/secret, and every community request is
// then HMAC-signed. Replaces the old static x-api-key scheme.
...
// SetCommunityVerificationHandlers wires how the verification challenge URL
// is opened (Wails BrowserOpenURL, OS shell, or a frontend event in serve
// mode) and how the app window is brought back after the user completes it.
...
func runCommunityVerification(record *communitySessionRecord) (string, error) {
	...
	Dbgf("Community verification required — opening %s\n", challengeURL.Host)
